...
export async function POST(req: NextRequest) {
  try {
    ...
    const thirdWaveOccurred = typeof body.thirdWaveOccurred === 'boolean' ? body.thirdWaveOccurred : null;
    const slopeCount = Number.isFinite(Number(body.slopeCount)) && body.slopeCount !== null && body.slopeCount !== ''
      ? Math.round(Number(body.slopeCount))
      : null;
    const shapeType: ShapeType | null =
      typeof body.shapeType === 'string' && (SHAPE_TYPES as readonly string[]).includes(body.shapeType)
        ? (body.shapeType as ShapeType)
        : null;
    ...
    const baseLow = parseExtraPoint(body, 'baseLow');
    const parabolicStart = parseExtraPoint(body, 'parabolicStart');
    ...
    const row = await prisma.parabolicCase.create({
      data: {
        ...
        slopeCount,
        shapeType: shapeType ? (PrismaShapeType[shapeType] as PrismaShapeType) : null,
        ...volumes,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
